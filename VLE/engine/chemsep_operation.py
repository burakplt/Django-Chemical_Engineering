...
def create_chemical(search_tag):
    ...
    "EnergyLJ","RacketParameter","FullerVolume","Parachor","SpecificGravity","LibraryIndex",
    ...
    "Asog","GCmethod","Umr","ModifiedUnifac")
    ...
    chemsep_xml = 'C:\\Users\\burak\\Desktop\\py_tests\\Databases\\chemsep.xml'
    ...
    for elem in root.iter(tag='compound'): #Find compound in XML file and locate
        if elem.find('CAS').get('value') == search_tag:
            compound  = elem
            name = compound[1].get('value')
            substance = Chemical(name, search_tag) #create a Chemical class object
...
def create_chemical2():
    ...
    chemsep_xml = BASE_DIR+"/VLE/engine/Databases/chemsep.xml"
    tree = ET.parse(chemsep_xml)
    root = tree.getroot()
    compound="" #Place holder for compound 
    ...
    for elem in root.iter(tag='compound'): #Find compound in XML file and locate
        ...
        file_name = lib
        pickle_out = open(BASE_DIR+"/VLE/engine/chemicals/"+file_name+".pickle", "wb")
...
def get_chemical(lib_index):
    ...
    path = BASE_DIR+"/VLE/engine/chemicals/"+lib_index+".pickle"
